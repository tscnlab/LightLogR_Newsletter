...
function buildSkyScene() {
  ...
  const flareSpecs = [
    { distance: 0.2, size: 16, hue: 42, opacity: 0.18, phase: 0.0 },
    { distance: 0.38, size: 20, hue: 70, opacity: 0.15, phase: 1.1 },
    { distance: 0.56, size: 26, hue: 195, opacity: 0.10, phase: 2.4 },
    { distance: 0.78, size: 34, hue: 310, opacity: 0.07, phase: 3.1 },
    { distance: 1.05, size: 46, hue: 24, opacity: 0.05, phase: 4.0 }
  ];
  ...
  function animateSun(timestamp) {
    ...
    const sunCenterY = y + sunSize*1.75;
    const centerX = viewportW * (0.9-progress*1.15);
    const centerY = viewportH * 0.8;
    ...
    flareOrbs.forEach((orb, index) => {
      ...
      const alpha = Math.min(0.33, Math.max(0.02, spec.opacity + 0.05 * offset));
    ...
    const streakPulse = 0.15 + 0.18 * Math.sin(pulse + 0.4);
