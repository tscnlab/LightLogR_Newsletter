...
function buildSkyScene() {
  ...
  const flareSpecs = [
    { distance: 0.22, size: 18, hue: 45, opacity: 0.35, phase: 0.0 },
    { distance: 0.48, size: 26, hue: 190, opacity: 0.25, phase: 1.7 },
    { distance: 0.72, size: 34, hue: 310, opacity: 0.18, phase: 3.1 }
  ];

  const flareOrbs = flareSpecs.map((spec) => {
    const orb = document.createElement('div');
    orb.className = 'flare-orb';
    orb.style.background = `hsla(${spec.hue}, 85%, 78%, ${spec.opacity})`;
  ...
  function animateSun(timestamp) {
    ...
    flareOrbs.forEach((orb, index) => {
      ...
      orb.style.background = `hsla(${spec.hue}, 85%, 78%, ${alpha})`;
